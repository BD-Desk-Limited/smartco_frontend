...
const SideBar = () => {
    ...
    const { selectedMenu, setSelectedMenu } = UseAdminMenu();
    ...
    const handleMenuClick = (menu) => {
        ...
        router.push(`/pages/dashboard/admin${menu.link}`);
    };
        ...
        <div className='h-full flex flex-col justify-between w-full text-text-white'>
            ...
            <div className='flex flex-col gap-5 py-5 mx-5 border-t border-text-white justify-between'>
                <div className='flex flex-col gap-2'>
                    <div className='flex flex-row items-center gap-1 cursor-pointer hover:bg-error hover:text-text-white rounded-md py-2' onClick={logOut}>
                ...
                <div>
                    <div className='flex flex-row items-center py-2 w-full text-center'>    
                        <Image src='/assets/profile.png' alt='settings' width={15} height={15}/>
                        {isOpen &&<p className='text-sm'>{user?.role?.charAt(0).toUpperCase()+user?.role?.slice(1)}</p>}
                    </div>
                </div>
            </div>
        </div>
