...
const BulkUserUpload = ({ pageDescription }) => {
  ...
  const collateErrors = (data) => {
    ...
    data.forEach((user, index) => {
      ...
      if (!user.staffId) {
        error.staffId.push({
          error: `user staffId is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(user.staffId).passed) {
        error.staffId.push({
          error: `${verifyInputText(user.staffId).message} on row ${index + 1}`,
          row: index + 1,
        });
      }
      ...
      if (!user.fullName) {
        error.fullName.push({
          error: `user full name is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(user.fullName).passed) {
        error.fullName.push({
          error: `${verifyInputText(user.fullName).message} on row ${index + 1}`,
          row: index + 1,
        });
      }
      ...
      if (!user.email) {
        error.email.push({
          error: `user email is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyEmail(user.email).passed) {
        error.email.push({
          error: `${verifyEmail(user.email).message} on row ${index + 1}`,
          row: index + 1,
        });
      }
      ...
      if (!['seller', 'manager'].includes(user.role)) {
        error.role.push({
          error: `user role is required and must be either 'seller' or 'manager' please check your spelling for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(user.role).passed) {
        error.role.push({
          error: `${verifyInputText(user.role).message} on row ${index + 1}`,
          row: index + 1,
        });
      }
      ...
      if (!user.branch || user.branch.length === 0) {
        error.branch.push({
          error: `user branches are required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!user.branch.every((branch) => verifyInputText(branch).passed)) {
        user.branch.forEach((branch, branchIndex) => {
          if (!verifyInputText(branch).passed) {
            error.branch.push({
              error: `${verifyInputText(branch).message} for branch name on row ${index + 1}`,
              row: index + 1,
            });
          }
        });
      ...
      if (user.phoneNumber && !verifyPhoneNumber(user.phoneNumber).passed) {
        error.phoneNumber.push({
          error: `${verifyPhoneNumber(user.phoneNumber).message} on row ${index + 1}`,
          row: index + 1,
        });
      }

      // Check for duplicate staffId.
      const staffIdDuplicateIndex = data.findIndex(
        (u, i) => u.staffId === user.staffId && i !== index
      );
      if (user.staffId && staffIdDuplicateIndex !== -1) {
        error.duplicate.push({
          error: `Duplicate staffId found for row ${index + 1} and ${staffIdDuplicateIndex + 1}`,
          row: index + 1,
        });
      }

      // Check for duplicate email.
      const emailDuplicateIndex = data.findIndex(
        (u, i) => u.email === user.email && i !== index
      );
      if (user.email && emailDuplicateIndex !== -1) {
        error.duplicate.push({
          error: `Duplicate email found for row ${index + 1} and ${emailDuplicateIndex + 1}`,
          row: index + 1,
        });
      }
      ...
      if (user.phoneNumber) {
        const duplicateIndex = data.findIndex(
          (u, i) => u.phoneNumber === user.phoneNumber && i !== index
        );
        if (duplicateIndex !== -1) {
          error.duplicate.push({
            error: `Duplicate phone number found for row ${index + 1} and ${duplicateIndex + 1}`,
            row: index + 1,
          });
        }
      }
    ...
    try {
      ...
      if (file) {
        if (!validateFile(file) || !file.name.includes(validFileName)) {
          setError(
            'File does not meet the required specification. Download required Template'
          );
          setUploading(false);
          return;
        }
      ...
      reader.onload = (e) => {
        ...
        if (jsonData.length !== 0) {
          ...
          if (!hasRequiredHeadings) {
            setError(
              `Please upload the correct Excel file we have provided: "${validFileName}"`
            );
            setUploading(false);
            return;
          }
          ...
          const usersData = filteredData.map((user) => {
            return {
              ...
              branch: user.Branches
                ? user.Branches.split(',').map((branch) => branch.trim())
                : [],
          ...
          if (
            !Object.keys(uploadedDataSample).includes(
              'Staff_ID' &&
                'Full_Name' &&
                'Email' &&
                'Role' &&
          ...
          setError(
            'No data found in the uploaded file, please check the file and try again.'
          );
          setUploading(false);
        }
      };
    ...
    <div className="">
          ...
          <div className="bg-background-1 p-5 my-2 mx-5 rounded-md h-full flex flex-col justify-center items-center">
            <div className="bg-text-white p-5 rounded-md min-h-[50vh] w-[40vw] flex flex-col justify-center items-center gap-2">
              ...
              <span className="text-text-gray text-sm">
                create user accounts within your organisation in bulk
              </span>
              <span className="text-error text-center font-semibold my-2 text-sm">
                Please check your downloads folder for the template. Fill the
                template, and upload your filled excel template
              </span>
                ...
                {file ? (
                  ...
                  <div className="flex flex-col items-center">
                    <Image
                      ...
                      className=""
                    />
                    <div className="text-text-gray mb-2">
              ...
              {error && (
                <p className="text-error mt-1 text-base text-center">{error}</p>
              )}
              {!error ? (
                <button
            ...
            {uploading && (
              <div className="mb-20 mt-5 flex flex-col justify-center items-center gap-2 w-[40vw]">
                <div className="text-text-black font-bold text-left text-base w-full">
                  Upload Progress
                </div>
      ...
      {reviewUpload && (
        <div className="inset-0 bg-black bg-opacity-70 fixed z-50 flex justify-center items-center">
          {errorData.every((error) =>
            Object.values(error).every((value) => value.length === 0)
          ) ? (
            <SuccessModal
            ...
            <ErrorModal
              ...
              subText={
                'Some rows were not uploaded due to errors. please review the errors and try again'
              }
