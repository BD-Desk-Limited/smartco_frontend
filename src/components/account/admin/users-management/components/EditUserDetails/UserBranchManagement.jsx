...
const UserBranchManagement = ({ updatedUserData, setUpdatedUserData }) => {
  // Always derive branches from updatedUserData
  const branches = updatedUserData?.branch || [];
  const [allBranches, setAllBranches] = React.useState([]);

  // No need to sync branches state from updatedUserData
  ...
  const addBranch = (branchId) => {
    ...
    if (
      selectedBranch &&
      !branches.some((branch) => branch._id === selectedBranch._id)
    ) {
      setUpdatedUserData((prev) =>
        prev?.branch
          ? { ...prev, branch: [...prev.branch, selectedBranch] }
          : { ...prev, branch: [selectedBranch] }
      );
    }
  ...
  const branchesNotAddedYet = allBranches.filter(
    (branch) => !branches.map((b) => b._id).includes(branch._id)
  );
    ...
    <div className="bg-white shadow-md rounded-lg p-5 mr-10 relative h-full">
      ...
      <div>
        <p className="text-text-gray text-sm mt-2">
          Branches associated with the user.
        </p>
        {branches?.length < 1 ? (
          ...
          <ul className="w-full px-2 rounded-lg shadow-sm my-2 py-5 grid grid-cols-4 gap-2 min-h-[150px] relative">
            <span className="absolute top-0 right-2 text-brand-blue text-sm">
              Number of branches - {branches && branches.length}
            </span>
            {branches &&
              branches.map((branch) => (
                <li
      ...
      <div className="mt-5">
        ...
        >
          ...
          {allBranches &&
            branchesNotAddedYet.map((branch) => (
