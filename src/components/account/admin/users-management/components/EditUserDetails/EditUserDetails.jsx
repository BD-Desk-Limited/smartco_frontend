...
const EditUserDetails = ({ pageDescription, userData }) => {
  ...
  const handleOpenModal = () => {
    ...
    if (
      ...
      !verifyPhoneNumber(updatedUserData.phoneNumber).passed
    ) {
      inputValidationErrors.push(
        verifyPhoneNumber(updatedUserData.phoneNumber).message +
    ...
    if (updatedUserData.team && !validateTeam.passed) {
      inputValidationErrors.push(
        validateTeam.message + ' Please select a valid team name.'
      );
    }
  ...
  const handleUserDataUpdate = async () => {
    ...
    try {
      ...
      const response = await updateUserService(
        userData._id,
        body,
        updatedUserData?.imageFile || null
      );
    ...
    <div className="h-screen overflow-y-auto no-scrollbar relative">
      ...
      <div className="flex flex-row max-h-[85vh] overflow-y-auto scrollbar-thin gap-0 w-full h-full relative my-2">
        ...
        <div className="flex flex-col h-full w-full overflow-y-auto no-scrollbar relative">
          <h1 className="px-5 text-text-black">
            Click on any field to update user details...
          </h1>
          ...
          <div className="w-full flex justify-center items-center gap-10 my-2 px-5">
            <Button onClick={handleOpenModal} text={'Update User Details'} />
            <Button
      ...
      {openConfirmationModal && (
        <div className="inset-0 z-50 flex justify-center items-center fixed bg-black bg-opacity-80">
          <WarningModal
            ...
            button1Style={
              'text-white hover:bg-blue-shadow5 rounded-md px-4 py-2'
            }
            button2Style={
              'bg-gray-shadow5 text-text-black hover:bg-gray-shadow4 rounded-md px-4 py-2'
            }
            loading={loading}
      ...
      {success && (
        <div className="inset-0 z-50 flex justify-center items-center fixed bg-black bg-opacity-80">
          <SuccessModal
            ...
            buttonText={'OK'}
            buttonStyle={
              'text-white bg-brand-blue hover:bg-blue-shadow5 rounded-md px-4 py-2'
            }
          />
        </div>
      )}
