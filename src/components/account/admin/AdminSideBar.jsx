...
const SideBar = ({ selectedMenu }) => {
  ...
  return (
    <div className={`bg-brand-blue h-screen flex flex-col ${isOpen? 'w-48':'w-20'}`}>
        <div className='flex justify-between items-center px-4 pt-5 pb-16'>
            <div className='flex flex-row justify-center items-center'>
                <Image src={'/assets/logo_white.png'} alt='logo' width={30} height={30}/>
                {isOpen && <Image src={'/assets/logo_long.png'} alt='logo' width={80} height={20}/>}
            </div>
            <p onClick={()=>setIsOpen(!isOpen)} className='cursor-pointer'>
                <Image src={isOpen? '/assets/arrow_left.png':'/assets/arrow_right.png'} alt='close' width={10} height={10}/>
            </p>
        </div>
        <div className='h-full flex flex-col justify-between w-full text-text-white'>
            ...
            { menuList.map((menu, index) => (
                ...
                >
                    <div className='flex flex-row items-center' title={!isOpen? `${menu?.title}` : ''}>
                        <Image src={selectedMenu?.name === menu.name? menu.iconActive : menu.icon} alt={menu?.name} width={15} height={15} className={selectedMenu?.name === menu.name?'':'bg-brand-blue'}/>
            ...
            <div className='flex flex-col gap-5 py-5 mx-5 border-t border-text-white justify-between'>
                <div className='flex flex-col gap-2'>
                    <div className='flex flex-row items-center gap-1 cursor-pointer hover:bg-error hover:text-text-white rounded-md py-2' title={!isOpen? 'Logout' : ''} onClick={logOut}>
                        <Image src='/assets/logout.png' alt='logout' width={15} height={15}/>
                        {isOpen &&<p className='text-sm'>Logout</p>}
                    </div>
                    {user?.superAdmin &&
                        <div className='flex flex-row items-center gap-1 cursor-pointer hover:bg-error hover:text-text-white rounded-md py-2' title={!isOpen? 'deactivavte account' : ''}>
                ...
                <div>
                    <div className='flex flex-row items-center py-2 w-full text-center'>    
                        <Image src='/assets/profile.png' alt='profile' width={20} height={15}/>
