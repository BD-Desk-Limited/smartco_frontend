...
const ProductSidePreview = ({
  ...
  const taxInfo =
    selectedProduct?.productTax?.length > 0
  ...
  return (
    <div className="max-h-[99%] min-h-[99%] overflow-y-auto no-scrollbar bg-brand-blue rounded-md text-text-white px-3 relative">
      <h2 className="text-base font-semibold sticky top-0 py-2 bg-brand-blue">
        {selectedProduct?.name || 'Product Preview'}
      </h2>
        ...
        <ul className="max-h-32 overflow-y-auto scrollbar-thin">
          {components.length > 0 ? (
            components.map((component, index) => (
              <li
                key={index}
                className="text-sm font-mono px-1 flex flex-col gap-1 justify-between"
              >
                    ...
                    component.materialChoices.map((mat, idx) => (
                      <span key={idx} className="flex flex-col">
                        <span className="flex flex-row justify-between">
                          ...
                          <span>
                            {mat.quantity} {getMaterialUnits(mat.material)}
                          </span>
                        </span>
        ...
        <ul className="max-h-32 overflow-y-auto scrollbar-thin">
          {selectedProduct?.pricing && selectedProduct.pricing.length > 0 ? (
            selectedProduct.pricing.map((price, index) => (
              <li
                key={index}
                className="text-sm font-mono px-1 flex flex-row gap-1 justify-between"
              >
      ...
      <div className="text-sm flex flex-col gap-2 my-2">
        <hr />
        <ul className="max-h-32 overflow-y-auto scrollbar-thin">
          <h2 className="sticky top-0 font-semibold bg-brand-blue py-0.5">
            Tax Information
          </h2>
          {taxInfo.length > 0 ? (
            taxInfo.map((tax, idx) => (
              <li
                key={tax._id || idx}
                className="text-sm font-mono px-1 flex flex-row gap-1 justify-between my-1 bg-blue-shadow4"
              >
                <span>
                  {findTaxBandDetails(tax.taxBand)
                    ? findTaxBandDetails(tax.taxBand).name
                    : 'N/A'}
                </span>
                <span>
                  {`${tax.taxPercentage || 0}% `}
            ...
            <p className="text-sm italic text-center py-4">
              No additional tax information set!!
            </p>
          )}
        </ul>
        <hr />
...
export default ProductSidePreview;
