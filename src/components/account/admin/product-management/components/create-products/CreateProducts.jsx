...
import ProductSidePreview from './ProductSidePreview';
...
const CreateProducts = ({ pageDescription }) => {
  ...
  const handleOpenWarning = () => {
    ...
    for (const product of products) {
      if (!product.name) {
        setError([
          `Product name is required for product number ${products.indexOf(product) + 1}.`,
        ]);
        setLoading(false);
        return;
      }
      ...
      if (!validName.passed) {
        setError([
          `${validName.message} for product number ${products.indexOf(product) + 1}.`,
        ]);
        setLoading(false);
        return;
      }
      ...
      if (!product.description) {
        setError([
          `Product description is required for ${product.name ? product.name : 'this'} product number ${products.indexOf(product) + 1}.`,
        ]);
        setLoading(false);
        return;
      }
      ...
      if (!validDescription.passed) {
        setError([
          `${validDescription.message} for product description of ${product.name ? product.name : 'this'} product number ${products.indexOf(product) + 1}.`,
        ]);
        setLoading(false);
        return;
      }
      ...
      if (!product.category || product.category.trim() === '') {
        setError([
          `Product category is required for ${product.name ? product.name : 'this'} product number ${products.indexOf(product) + 1}.`,
        ]);
        setLoading(false);
        return;
      }
      ...
      if (
        ...
        setError([
          `At least one component with material choices is required for ${product.name ? product.name : 'this'} product number ${products.indexOf(product) + 1}.`,
        ]);
        setLoading(false);
        return;
      }
      ...
      if (
        !product.components.every((component) =>
          component.materialChoices.every(
            (material) => material.quantity && material.quantity > 0
          )
        )
      ) {
        setError([
          `Please ensure all material choices for ${product.name ? product.name : 'this'} product number ${products.indexOf(product) + 1} have a quantity greater than zero.`,
        ]);
        setLoading(false);
        return;
      }
    ...
    for (const key in bandsError) {
      if (bandsError[key]) {
        setError([
          `Please resolve the band price errors for ${products[key].name ? products[key].name : 'product number ' + (parseInt(key) + 1)} before creating products.`,
        ]);
        setLoading(false);
        return;
      }
    ...
    if (duplicateNames.length > 0) {
      setError([
        `Duplicate product names found for: ${[...new Set(duplicateNames)].join(', ')}. Please ensure all product names are unique.`,
      ]);
      setLoading(false);
      return;
    }
  ...
  const handleCreateProduct = async (e) => {
    ...
    try {
      ...
      if (error) {
        setError([
          error ||
            'An error occurred while creating products, please try again.',
        ]);
      } else if (data) {
        setSuccess(true);
      ...
      setError([
        error || 'An error occurred while creating products, please try again.',
      ]);
    } finally {
