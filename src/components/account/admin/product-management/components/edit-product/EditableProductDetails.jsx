...
const EditableProductDetails = ({ pageDescription }) => {
  ...
  const handleOpenWarning = () => {
    ...
    if (!product.name) {
      setError(['Product name is required.']); setLoading(false); return;
    }
    const validName = verifyInputText(product.name);
    if (!validName.passed) { setError([validName.message]); setLoading(false); return; }

    if (!product.description) { setError(['Product description is required.']); setLoading(false); return; }
    const validDescription = verifyInputText(product.description);
    if (!validDescription.passed) { setError([validDescription.message]); setLoading(false); return; }

    if (!product.category || product.category.trim() === '') { setError(['Product category is required.']); setLoading(false); return; }

    if (!product.components || !product.components.every(c => c.materialChoices.length > 0)) { setError(['At least one component with material choices is required.']); setLoading(false); return; }

    if (!product.components.every(c => c.materialChoices.every(m => m.quantity && m.quantity > 0))) { setError(['All material choices must have a quantity greater than zero.']); setLoading(false); return; }

    for (const key in bandsError) {
      if (bandsError[key]) { setError(['Please resolve price/tax band errors before updating.']); setLoading(false); return; }
    }
    ...
    <div className='relative bg-background-1'>
            ...
            <div className="p-5 h-full flex flex-col gap-2 min-h-[70vh] max-h-[80vh] overflow-y-auto no-scrollbar relative">
              ...
              <div className='bg-text-white p-2 sticky top-0 z-10 w-full flex flex-row items-center justify-between mb-5'>
                <h3 className='flex flex-row items-center gap-4'>
                  <span className='text-lg font-semibold flex flex-col'>
                    <span>{product?.name ? (product.name[0]?.toUpperCase() + product.name.slice(1)) : 'Unnamed Product'}</span>
                    <span className='text-xs text-text-gray py-1'>ID: {product?._id}</span>
                  </span>
                </h3>
                <ul className='flex flex-row gap-3 list-none'>
                  <li>
                    <button onClick={handleOpenWarning} className='border border-brand-blue text-text-gray p-1 rounded-lg flex flex-row items-center gap-1 hover:bg-brand-blue hover:text-text-white transition'>
                      <Image src={`/assets/edit.png`} alt={`Save`} width={16} height={16} />
                      Save Changes
                    </button>
                  </li>
                </ul>
              ...
              <div className='flex flex-row w-full px-5 gap-5'>
                  ...
                  <div className='bg-white shadow-lg p-2 rounded-lg flex flex-col gap-2'>
                    <div className='flex items-center justify-center'>
                      <span className={`${product?.image ? 'bg-gray-shadow8 p-5' : 'bg-gray-200 p-5'} rounded-full flex items-center justify-center`}>
                        <Image src={product?.image ? product.image.url : '/assets/shopping.png'} alt={product?.name || 'Product Image'} className='w-24 h-24 object-cover rounded-full' width={96} height={96}/>
                      </span>
                    </div>
                    <button onClick={()=>setOpenComponentsOverlay(false) || setOpenPriceOverlay(false) || setOpenPriceOverlay(false)} className='hidden'></button>
                    <button onClick={()=>setOpenComponentsOverlay(false)} className='hidden'></button>
                    <button onClick={()=>setOpenComponentsOverlay(false)} className='hidden'></button>
                    <button onClick={()=>setOpenComponentsOverlay(false)} className='hidden'></button>
                    {/* Add picture overlay trigger */}
                    <button onClick={()=> setOpenComponentsOverlay(false) || setOpenPriceOverlay(false)} className='hidden'></button>
                    <button onClick={()=> setOpenComponentsOverlay(false)} className='hidden'></button>
                    {/* Name */}
                    <div className='flex flex-col'>
                      <label className='text-sm text-text-gray'>Name</label>
                      <input value={product.name || ''} onChange={(e)=> setProduct(prev => ({...prev, name: e.target.value}))} className='border border-gray-border rounded-md p-2 text-sm' />
                    </div>
                    {/* Category */}
                    <div className='flex flex-col'>
                      <label className='text-sm text-text-gray'>Category</label>
                      <select value={product.category || ''} onChange={(e)=> setProduct(prev => ({...prev, category: e.target.value}))} className='border border-gray-border rounded-md p-2 text-sm'>
                    ...
                    <div className='flex flex-col'>
                      ...
                      <textarea value={product.description || ''} onChange={(e)=> setProduct(prev => ({...prev, description: e.target.value}))} className='border border-gray-border rounded-md p-2 text-sm h-24'/>
                    </div>
                  </div>
                </div>
                        ...
                        <tbody className='border-b'>
                          {product.pricing.map((band, i) => (
                            <tr key={i}>
                              ...
                              <td className='p-1'>{companyDetails?.currency?.symbol || '$'}{Number(band.price || 0).toFixed(2)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                      ...
                      product.components.map((comp, idx) => (
                        <div key={idx} className='border rounded p-2 my-1'>
                          <div className='flex justify-between'>
                            <span>{comp.categoryName || 'Unnamed Category'}</span>
                            <span className='text-text-gray'>Alternatives: {comp.materialChoices?.length || 0}</span>
                          </div>
                        </div>
                      ))
                    ...
                    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70'>
                      <div className='bg-brand-blue rounded-md shadow-lg w-[50vw] max-h-[85vh] overflow-y-auto p-2'>
                        <div className='flex justify-end'>
                          <button onClick={()=> setOpenComponentsOverlay(false)} className='bg-error text-white px-2 rounded'>Close</button>
                        </div>
                        <SelectComponents
                          products={products}
                          ...
                          closeComponentsDropdown={()=> setOpenComponentsOverlay(false)}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  {openPriceOverlay && (
                    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70'>
                      <div className='bg-brand-blue rounded-md shadow-lg w-[40vw] max-h-[85vh] overflow-y-auto p-2'>
                        <div className='flex justify-end'>
                          <button onClick={()=> setOpenPriceOverlay(false)} className='bg-error text-white px-2 rounded'>Close</button>
                        </div>
                        <SetPrices
                          products={products}
                          setProducts={setProducts}
                ...
                <div className='w-1/5 bg-white rounded-lg shadow-lg p-2'>
                  <h4 className='font-semibold text-brand-blue mb-2'>Quick Summary</h4>
                  <ul className='text-sm text-text-gray flex flex-col gap-1'>
                    <li>Prices: {product?.pricing?.length || 0}</li>
                    <li>Tax Bands: {product?.productTax?.length || 0}</li>
                    <li>Components: {product?.components?.length || 0}</li>
                  </ul>
                </div>
              </div>
            </div>
      ...
      {error && error.length > 0 && (
        <div className='fixed bottom-4 right-4 bg-error bg-opacity-20 border border-error text-error p-3 rounded-lg z-50'>
          {error.map((e, idx) => (<p className='text-sm' key={idx}>{e}</p>))}
        </div>
      )}
    </div>
  );
