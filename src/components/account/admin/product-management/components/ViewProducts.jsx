...
const ViewProducts = ({ pageDescription }) => {
  ...
  const handleOpenEnableOrDisableModal = (e, productId) => {
    ...
    if (allSameStatus) {
      ...
      setEnableAndDisableErrors([
        'Please select products with the same status to enable or disable.',
      ]);
    }
  ...
  const handleEnableOrDisableProduct = async (productId) => {
    ...
    if (productId.length === 0) {
      setEnableAndDisableErrors([
        'Please select at least one product to enable or disable.',
      ]);
      setLoading(false);
      return;
    }
      ...
      if (response.error) {
        ...
        const updatedProducts = allProducts.map((product) => {
          if (productId.includes(product._id)) {
            return {
              ...product,
              isDisabled: productsStatus === 'active' ? true : false,
            };
          }
          return product;
        });
    ...
    try {
      ...
      if (response.data) {
        ...
        if (data.deletedProducts.length > 0) {
          const deletedProductsId = data.deletedProducts.map(
            (product) => product._id
          );
          setDeleteMessages([
            `${data.deletedProducts.length} products deleted successfully.`,
          ]);
          setAllProducts(
            allProducts.filter(
              (product) => !deletedProductsId.includes(product._id)
            )
          );
          setSelectedProduct([]);
          setDeleteErrors([]);
        ...
        if (data.productsNotDeleted.length > 0) {
          const productsNotDeletedIds = data.productsNotDeleted.map(
      ...
      if (response.error) {
        setDeleteErrors(
          [response.error] || ['Error deleting products, please try again.']
        );
      }
    } catch (error) {
                    ...
                    {filteredProducts?.length > 0 ? (
                      filteredProducts?.map((product) => (
                        <tr
                          ...
                          onClick={() =>
                            Router.push(
                              `/pages/account/admin/product-management/product-details?id=${product._id}`
                            )
                          }
                        >
                          <td
                          ...
                          >
                            ...
                            <Image
                              src={
                                product?.imageURL || '/assets/shopping-bag.png'
                              }
                            ...
                            <span className="flex flex-row items-center gap-2 justify-around">
                              <button>
                                <Image
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation(); // Stop event propagation
                                    Router.push(
                                      `/pages/account/admin/product-management/edit-product?id=${product._id}`
                                    );
                                  }}
                                />
                              </button>
                              ...
                              <button className="">
                                <Image
                                  ...
                                  onClick={(e) =>
                                    handleOpenDeleteModal(e, [product._id])
                                  }
                                  className="cursor-pointer"
                                />
                              </button>
                ...
                {selectedProduct.length > 0 && (
                  <div className="flex justify-end mt-4 sticky bottom-0 right-5 gap-5">
                    <div className="bg-brand-green text-sm text-text-white p-2 rounded-md flex items-center gap-2">
                      ...
                      <button
                        onClick={(e) =>
                          handleOpenEnableOrDisableModal(e, selectedProduct)
                        }
                      ...
                      <button
                        onClick={(e) =>
                          handleOpenDeleteModal(e, selectedProduct)
                        }
                        className="text-brand-green px-2 py-1 rounded text-sm hover:scale-105"
                      >
                        <Image
      ...
      {openDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70">
          <DeleteModal
            ...
            title={
              selectedProduct.length > 1 ? 'Delete Products' : 'Delete Product'
            }
          ...
          <ExportContent
            metadata={{
              ...
              category: selectedCategory
                ? [getSelectedCategoryNameById(selectedCategory)]
                : ['All'],
            }}
