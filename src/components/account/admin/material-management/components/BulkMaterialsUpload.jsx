...
const BulkMaterialsUpload = ({ pageDescription }) => {
  ...
  const { setBulkMaterialData, setErrorData, errorData } =
    useBulkMaterialUpload();
  ...
  const collateErrors = (data) => {
    ...
    data.forEach((material, index) => {
      ...
      if (!material.name) {
        error.name.push({
          error: `Material name is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(material.name).passed) {
        error.name.push({
          error: `${verifyInputText(material.name).message} on row ${index + 1}`,
          row: index + 1,
        });
      }

      if (!material.materialType) {
        error.materialType.push({
          error: `Material type is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!['raw-material', 'packaging'].includes(material.materialType)) {
        error.materialType.push({
          error: `Material type must be either 'raw-material' or 'packaging-material' for row ${index + 2}. Please check the spellings on the template`,
          row: index + 1,
        });
      }
      ...
      if (!material.category) {
        error.category.push({
          error: `Material category is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(material.category).passed) {
        error.category.push({
          error: `${verifyInputText(material.category).message} on row ${index + 1}`,
          row: index + 1,
        });
      }

      if (!material.unitOfMeasurement) {
        error.unitOfMeasurement.push({
          error: `Material unit of measurement is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(material.unitOfMeasurement).passed) {
        error.unitOfMeasurement.push({
          error: `${verifyInputText(material.unitOfMeasurement).message} on row ${index + 1}`,
          row: index + 1,
        });
      }
      ...
      if (!material.description) {
        error.description.push({
          error: `Material description is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(material.description).passed) {
        error.description.push({
          error: `${verifyInputText(material.description).message} on row ${index + 2}`,
          row: index + 1,
        });
      }
        ...
        if (jsonData.length !== 0) {
          ...
          if (
            !Object.keys(uploadedDataSample).includes(
              'Material Name' && 'Type' && 'Category' && 'Unit' && 'Description'
            )
          ) {
            setError(
              `Please upload the correct Excel file we have provided ${validFileName}`
          ...
          <div className="bg-background-1 p-5 my-2 mx-5 rounded-md h-full flex flex-col justify-center items-center">
            <div className="bg-text-white p-5 rounded-md min-h-[50vh] w-[40vw] flex flex-col justify-center items-center gap-2">
              ...
              <span className="text-text-gray text-sm">
                upload bulk materials to your stores
              </span>
