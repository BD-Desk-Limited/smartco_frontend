      ...
      {isOpen? (
        ...
        >
          ...
          {submenus.map((submenu, index) => (
            <div key={index} onClick={() => handleSelectSubMenu(submenu)} className={`p-1 rounded-sm mx-0.5 cursor-pointer hover:bg-text-white hover:text-brand-green ${selectedSubMenu.name === submenu.name ? 'bg-text-white text-brand-green' : ''}`}>
              {submenu.name}
            </div>
          ))}
        </motion.div>
      ):(
        <div onClick={()=>setIsOpen(!isOpen)} className='cursor-pointer py-2 px-3 bg-brand-green rounded-md m-0.5'>
          <Image 
