...
const CreateMaterial = ({ pageDescription }) => {
  ...
  const handleCreateMaterial = async (e) => {
    ...
    try {
      ...
      //api call to create material goes here!!!

    } catch (error) {
      console.error('Error:', error);
      setError('error creating material, please try again!!!');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      ...
      <div className="flex flex-row gap-0 w-full h-full">
        <div className="min-w-fit">
          <MaterialSidebar selectedSubMenu={selectedSubMenu} />
        </div>
      ...
      {openModal && (
        <div className="inset-0 bg-black bg-opacity-70 fixed z-50 flex justify-center items-center flex-col">
          ...
          <BulkMaterialUploadModal closeModal={() => setOpenModal(false)} />
        </div>
      )}
    </div>
  );
