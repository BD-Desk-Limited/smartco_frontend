...
const ViewMaterialDetails = ({ materialData, pageDescription }) => {
  ...
  const batchesWithStock = materialData.batches?.filter(
    (batch) => batch.quantity > 0
  );
  const totalStock = batchesWithStock?.reduce(
            ...
            <div className="flex flex-col gap-2 w-full">
              ...
              <div className="flex flex-row justify-between gap-2">
                <div className="w-[50%] flex flex-col gap-2">
                  <p className="h-28 overflow-y-auto scrollbar-thin">
              ...
              <div>
                ...
                <span className="">
                  {batchesWithStock?.length > 0 ? (
                      ...
                      <table className="w-full border-collapse rounded-md">
                        ...
                        <tbody>
                          {batchesWithStock?.map((stockLevel) => (
                            <tr
                              key={stockLevel._id}
                              className="w-full flex flex-row justify-between gap-5 border-b-2 border-b-text-white bg-blue-shadow10 text-sm px-5 py-1"
                            >
                    ...
                    <span className="p-10 font-semibold text-error">
                      No stock available
                    </span>
                  )}
                </span>
