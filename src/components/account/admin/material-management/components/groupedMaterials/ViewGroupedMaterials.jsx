...
const ViewGroupedMaterials = ({ pageDescription }) => {
  ...
  const [allMaterialsIncludingUngrouped, setAllMaterialsIncludingUngrouped] =
    React.useState([]);
    ...
    const fetchGroupedMaterials = async () => {
      try {
        ...
        if (response.data) {
          ...
          const groupedMaterials = data.filter(
            (material) =>
              material.isGroup === true && material.isDeleted === false
          );
  ...
  React.useEffect(() => {
    const filtered =
      (Array.isArray(groupedMaterials) &&
        groupedMaterials?.filter(
          (material) =>
  ...
  const allCategories = [
    ...new Set(groupedMaterials.map((material) => material.category?.name)),
  ];
  const allMaterialTypes = [
    ...new Set(groupedMaterials.map((material) => material.materialType)),
  ];
    ...
    try {
      ...
      if (response.data) {
        ...
        if (data.deletedMaterials.length > 0) {
          const deletedMaterialsId = data.deletedMaterials.map(
            (material) => material._id
          );

          setDeleteMessages([
            `${data.deletedMaterials.length} materials deleted successfully.`,
          ]);
          setAllMaterialsIncludingUngrouped(
            allMaterialsIncludingUngrouped.filter(
              (material) => !deletedMaterialsId.includes(material._id)
            )
          );
        ...
        if (data.materialsWithStock.length > 0) {
          ...
          errorTypes.forEach((errorType) => {
            const errorMaterials = data.materialsWithStock.filter(
              (material) => material.message === errorType
            );
            errorWithStocksUnderIt = [
              ...errorWithStocksUnderIt,
              { errorType, errorMaterials },
            ];
          });
      ...
      if (response.error) {
        setDeleteErrors(
          [response.error] || ['Error deleting materials, please try again.']
        );
      }
    } catch (error) {
  ...
  const getMaterialNameAndUnitById = (id) => {
    const material = allMaterialsIncludingUngrouped.find(
          ...
          <div className="bg-white p-5 mx-5 my-2 rounded-md h-full flex flex-col gap-5">
            <div className="flex flex-row gap-5 w-full justify-between items-center">
              ...
              <div className="flex flex-row gap-2 text-sm min-w-fit text-text-gray">
                ...
                <button>
                  <Link
                    href="/pages/account/admin/manage-materials/create-grouped-material"
                      ...
                      <>
                        {filteredMaterials.map((material) => (
                          <tr
                            ...
                            onClick={() =>
                              Router.push(
                                `/pages/account/admin/manage-materials/view-material-details?id=${material._id}`
                              )
                            }
                          ...
                          >
                            ...
                            <td className="px-4 py-2 flex flex-row w-full justify-between">
                              ...
                              />
                              <Link
                                href={`/pages/account/admin/manage-materials/edit-grouped-material?id=${material._id}`}
                                title="Edit Material"
                              >
                                <Image
                                  src="/assets/edit.png"
          ...
          <ExportContent
            ...
            data={filteredMaterials.map((material, index) => {
              return {
                ...
                'Component Materials': material.components?.map(
                  (componentMaterial) => {
