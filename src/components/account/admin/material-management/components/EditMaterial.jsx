...
const EditMaterial = ({ materialData, pageDescription }) => {
  ...
  const handleCreateUnit = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        setUpdatedMaterialData({
          ...updatedMaterialData,
          unitOfMeasurement: response.data.unit,
        });
      }
    } catch (err) {
      console.error(err);
  ...
  const handleUpdateMaterial = async () => {
    ...
    handleValidation(
      updatedMaterialData.materialType,
      'Please select a valid material type'
    );
    handleValidation(
      updatedMaterialData.category.name,
      'Please select a valid material category'
    );
    handleValidation(
      updatedMaterialData.unitOfMeasurement.name,
        ...
        <div className="flex flex-col w-full h-full">
          <div className="bg-white p-5 mx-5 my-2 rounded-md h-full text-text-gray">
            ...
            <span className="text-sm">
              Edit this material in your stores, Click on any field to update
            </span>
              ...
              <form className="flex flex-row justify-between gap-10 w-[70%]">
                  ...
                  {createdNewCategory ? (
                    <div className="w-full text-center px-5">
                      <span className="text-center text-success text-sm w-full">
                        New category created successfully. New category will be
                        associated with this material when updated
                      </span>
                    </div>
                  ) : (
                    <>
                ...
                <div className="flex flex-col gap-5 w-full">
                  ...
                  {createdNewUnit ? (
                    <div className="w-full text-center px-5">
                      <span className="text-center text-success text-sm w-full">
                        New unit created successfully. New unit will be
                        associated with this material when updated
                      </span>
                    </div>
                  ) : (
            ...
            <div className="flex flex-row justify-center w-full gap-20 my-10 items-center">
              <Button
                text={'Cancel'}
                buttonStyle={
                  'text-sm px-5 border border-brand-blue  rounded-md'
                }
                onClick={() => {
                  setUpdatedMaterialData(materialData);
                  setNewMaterialImage(null);
      ...
      {showSuccessModal && (
        <div className="inset-0 fixed z-70 flex justify-center items-center bg-black bg-opacity-50">
          <SuccessModal
            ...
            buttonText={'close'}
            buttonStyle={
              'hover:bg-blue-shadow2 p-2 bg-brand-blue text-white rounded-md'
            }
            onClose={() =>
              router.push(
                '/pages/account/admin/manage-materials/view-materials'
