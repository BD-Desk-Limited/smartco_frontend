...
const EditBranch = ({ branchData, setBranchData, pageDescription }) => {
  ...
  const handleUpdateBranch = async () => {
    ...
    try {
      ...
      handleValidation(
        updatedBranchData.name,
        'Please enter a valid Branch name'
      );
      handleValidation(
        updatedBranchData.address,
        'Please enter a valid Branch address'
      );
      handleValidation(
        updatedBranchData.email,
        'Please enter a valid Branch email'
      );
      handleValidation(
        updatedBranchData.phoneNumber,
        'Please enter a valid Branch phone number'
      );
      handleValidation(updatedBranchData.band, 'Please select a valid Band');
      handleValidation(
        updatedBranchData.taxBand?.name,
        'Please select a valid Tax Band'
      );
      handleValidation(
        updatedBranchData.openingHour,
        'Please select a valid Opening Time'
      );
      handleValidation(
        updatedBranchData.closingHour,
      ...
      setError(
        'An error occurred while updating the branch information. Please try again later.'
      );
    } finally {
      setLoading(false);
      setOpenSubmitModal(false);
        ...
        <div className="flex flex-col h-full text-text-gray relative">
          <div className="bg-white p-5 mx-5 my-2 rounded-md h-full flex flex-col gap-5">
            <h1 className="text-sm">
              Click on any field to update branch information
            </h1>
              ...
              <div className="flex flex-col gap-5 min-h-full p-5 rounded-lg bg-gray-shadow9 w-full">
                <div className="flex flex-row gap-1 items-center">
                  ...
                  <select
                    ...
                    className="border border-gray-border rounded-md p-2 text-sm"
                  >
                    {updatedBranchData && updatedBranchData?.band ? (
                      <option value={updatedBranchData?.band}>
                    ...
                    {companyBand.length > 0 ? (
                      companyBand
                        .filter((band) => band !== updatedBranchData?.band)
                        .map((band, index) => (
                ...
                <div className="flex flex-row gap-1 items-center w-full">
                  ...
                  {showTaxBandInfo ? (
                    <span
                      className="bg-text-white rounded-md text-sm text-brand-blue italic p-2"
                    ...
                    <>
                      <select
                        ...
                        onChange={(e) =>
                          setUpdatedBranchData({
                            ...updatedBranchData,
                            taxBand: companyTaxBands.find(
                              (taxBand) => taxBand.name === e.target.value
                            ),
                          })
                        }
                        className="border border-gray-border rounded-md p-2 text-sm"
                      >
                        {updatedBranchData &&
                        updatedBranchData?.taxBand?.name ? (
                          <option value={updatedBranchData?.taxBand.name}>
                            {updatedBranchData?.taxBand?.name}
                          </option>
                        ) : (
                        ...
                        {companyTaxBands.length > 0 ? (
                          companyTaxBands
                            ...
                            .map((taxBand, index) => (
                              <option key={index} value={taxBand?.name}>
                                {taxBand?.name}
                              </option>
                            ))
                        ) : (
                ...
                <div className="flex flex-col gap-3 bg-white shadow-md p-2 rounded-lg">
                  ...
                  <div className="flex flex-row gap-5">
                    ...
                    {/* Closing Time */}
                    <div className="flex flex-col gap-1">
                      <label className="text-sm text-error font-semibold">
                        Closing Time:
                      </label>
                      <TimePicker
                        onChange={(value) =>
                ...
                {updatedBranchData !== branchData && (
                  <div className="flex flex-row justify-center gap-10 w-full my-2">
                    <Button
                      ...
                      buttonStyle={
                        'bg-brand-blue text-white rounded-md p-2 w-fit'
                      }
                    />
