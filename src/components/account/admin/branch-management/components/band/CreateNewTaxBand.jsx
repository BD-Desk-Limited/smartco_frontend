...
const CreateNewTaxBand = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!newTaxBand.name) {
      setError('Please enter a valid name for the tax band');
      return;
    }
    if (!newTaxBand.rate) {
      setError('Please enter a valid rate for the tax band');
      return;
    }
    if (!newTaxBand.description) {
      setError('Please enter a valid description for the tax band');
      return;
    }
    ...
    if (new Date(newTaxBand.effectiveDate) < new Date(Date.now())) {
      inputValidationErrors.push('Effective date cannot be in the past.');
    }
    ...
    try {
      ...
      if (response.error) {
        setError(
          response.error || 'An error occurred while creating the tax band.'
        );
        return;
      }
      ...
      console.error(err);
      setError(
        'An error occurred while creating the tax band. Please try again later.'
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="w-[80vw] h-[80vh] relative bg-white rounded-lg shadow-lg flex flex-col p-5 items-center">
      ...
      <div className="w-full h-full flex flex-row shadow-lg rounded-lg p-1 border-2 border-gray-border">
        <form className="w-[50%] text-sm" onSubmit={handleSubmit}>
          <h2 className="text-brand-blue font-light w-full text-center">
            Create New Tax Band
          </h2>
          <div className="flex flex-col w-full gap-5 h-full items-center justify-center">
            <div className="flex flex-col gap-1 w-[80%]">
              <label className="text-text-gray w-fit" htmlFor="name">
                Tax Band Name
              </label>
              <input
                id="name"
            ...
            <div className="flex flex-col gap-1 w-[80%]">
              <label
                className="text-text-gray text-sm w-fit"
                htmlFor="rate"
              >{`Tax Rate (%)`}</label>
              <input
                id="rate"
                ...
                onChange={(e) =>
                  setNewTaxBand({ ...newTaxBand, rate: e.target.value })
                }
              />
            </div>
            <div className="flex flex-col gap-1 w-[80%]">
                ...
                <tbody className="text-brand-blue text-sm">
                  {taxBands?.length > 0 ? (
                    ...
                    <tr className="border-b border-gray-border hover:bg-gray-border cursor-pointer">
                      <td
                        className="py-10 font-semibold text-center"
                        colSpan={3}
                      >
                        You have no existing tax band please create a tax
                        band!!!
                      </td>
                    </tr>
                  )}
