...
const BulkBranchUpload = ({ pageDescription }) => {
  ...
  const { bulkbranchData, setBulkbranchData, errorData, setErrorData } =
    useBulkbranchUpload();
    ...
    data.forEach((branch, index) => {
      ...
      const passedTimeFormat = (time) => {
        ...
        return {
          ...
          message: passed
            ? ''
            : 'Invalid time format. Please use HH:MM AM or HH:MMAM format',
        };
      };
      ...
      if (!branch.name) {
        error.name.push({
          error: `branch name is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(branch.name).passed) {
        error.name.push({
          error: `${verifyInputText(branch.name).message} on row ${index + 1}`,
          row: index + 1,
        });
      }
      ...
      if (!branch.address) {
        error.address.push({
          error: `branch address is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(branch.address).passed) {
        error.address.push({
          error: `${verifyInputText(branch.address).message} for address on row ${index + 1}`,
          row: index + 1,
        });
      }
      ...
      if (!branch.branchId) {
        error.branchId.push({
          error: `branch ID is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(branch.branchId).passed) {
        error.branchId.push({
          error: `${verifyInputText(branch.branchId).message} for branch ID on row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!branch.email) {
        error.email.push({
          error: `branch email is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyEmail(branch.email).passed) {
        error.email.push({
          error: `${verifyEmail(branch.email).message} for email on row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!branch.phoneNumber) {
        error.phoneNumber.push({
          error: `branch phone number is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(branch.phoneNumber).passed) {
        error.phoneNumber.push({
          error: `${verifyInputText(branch.phoneNumber).message} for phone number on row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!branch.band) {
        error.band.push({
          error: `branch band is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(branch.band).passed) {
        error.band.push({
          error: `${verifyInputText(branch.band).message} for branch brand on row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!branch.taxBand) {
        error.taxBand.push({
          error: `branch tax band is required for row ${index + 1}`,
          row: index + 1,
        });
      }
      if (!verifyInputText(branch.taxBand).passed) {
        error.taxBand.push({
          error: `${verifyInputText(branch.taxBand).message} only enter the percentage figure for tax band on row ${index + 1}`,
          row: index + 1,
        });
      }
      if (branch.openingHour && !passedTimeFormat(branch.openingHour).passed) {
        error.openingHour.push({
          error: `${passedTimeFormat(branch.openingHour).message} for opening hour on row ${index + 1}`,
          row: index + 1,
        });
      }
      if (branch.closingHour && !passedTimeFormat(branch.closingHour).passed) {
        error.closingHour.push({
          error: `${passedTimeFormat(branch.closingHour).message} for closing hour on row ${index + 1}`,
          row: index + 1,
        });
      }
          ...
          <div className="bg-background-1 p-5 my-2 mx-5 rounded-md h-full flex flex-col justify-center items-center">
            <div className="bg-text-white p-5 rounded-md min-h-[50vh] w-[40vw] flex flex-col justify-center items-center gap-2">
              ...
              <span className="text-text-gray text-sm">
                upload bulk branches data
              </span>
              <span className="text-success font-semibold my-2 text-xs">
