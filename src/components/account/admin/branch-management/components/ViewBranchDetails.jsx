        ...
        <div className="flex flex-col w-full max-h-[85vh] overflow-y-auto scrollbar-thin">
          <div className="bg-white p-5 mx-5 my-2 rounded-md max-h-[90vh] overflow-y-auto scrollbar-thin flex flex-col gap-5 px-5 py-5 relative">
            <div className="absolute top-2 right-5 flex items-center justify-center gap-2">
              <span
                className={`w-5 h-5 rounded-full ${branchData.status === 'active' ? 'bg-brand-green' : 'bg-error'}`}
              ></span>
              {branchData.status && (
            ...
            <div className="flex flex-col gap-2 w-full my-8">
              ...
              <div className="flex flex-row gap-2 py-2 justify-between">
                <div className="flex flex-col gap-2 py-2">
                  <p className="text-sm text-text-gray">
                    Branch Band: <strong>{branchData?.band}</strong>
                  </p>
                  <p className="text-sm text-text-gray">
                    Created on:{' '}
                  ...
                  <p className="text-sm text-text-gray">
                    Created by: {branchData?.createdBy?.fullName}
                  </p>
                  <p className="text-sm text-text-gray">
                    Last updated:{' '}
                  ...
                  <p className="text-sm text-text-gray">
                    Last updated by: {branchData?.updatedBy?.fullName}
                  </p>
                  <p className="text-sm text-text-gray flex flex-row gap-2 items-center">
                    <span>
                      Tax Band: <strong>{branchData?.taxBand?.name}</strong>
                    </span>
                    <Image
                      src={'/assets/info.png'}
                      height={20}
                    ...
                    {seeTaxBandInfo && (
                      <span
                        className="absolute bg-white border border-brand-blue rounded-md p-1 w-[50%] z-50"
                        onMouseLeave={() => setSeeTaxBandInfo(false)}
                      >
                        <span className="text-sm text-text-gray">
                          Tax Band: {branchData?.taxBand?.description}
                        </span>
                      </span>
                    )}
                  </p>
                ...
                <div>
                  <Button
                    ...
                    buttonStyle={
                      'bg-brand-green hover:bg-green-shadow1 text-white'
                    }
                    onClick={() =>
                      router.push(
                        `/pages/account/admin/branch-management/edit-branch?id=${branchData._id}`
                      )
                    }
                  />
                </div>
        ...
        <div className="inset-0 fixed bg-black bg-opacity-60 z-50 flex justify-center items-center">
          <DeactivationModal
            ...
            message={`Are you sure you want to ${
              ...
              branchData.status === 'active'
                ? 'This would stop every activity in this branch.'
                : ''
            }`}
            buttonText={
              branchData?.status === 'active' ? 'Deactivate' : 'Activate'
