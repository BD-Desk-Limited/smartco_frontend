...
const AllProductsTable = ({
  ...
  React.useEffect(() => {
    let filteredDataForCurrentTable =
      table?.normalizedData?.[activeProductTable] || [];
  ...
  React.useEffect(() => {
    const normalised = fetchedDocuments.map((doc) => ({
      ...
      Category: doc.category?.name?.toLowerCase(),
      Price: doc.price,
    }));
    ...
    setTable((prev) => ({
      ...
      normalizedData: {
        ...prev.normalizedData,
        [activeProductTable]: normalised,
      },
    }));
  }, [fetchedDocuments, activeProductTable, setTable]);

  //extract table headers to be included as filter options and the ones to display in the table
  React.useEffect(() => {
    if (table?.normalizedData?.[activeProductTable]?.length > 0) {
      const headers = Object.keys(table.normalizedData[activeProductTable][0]);
  ...
  }, [table?.normalizedData, setTable, activeProductTable]);
